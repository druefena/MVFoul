...
class MultiDimStacker(nn.Module):
    def __init__(self,
        ...
        self.num_features = 1280 #num_3d_stack_proj * self.num_stacks# *3
    ...
    def forward_2d(self, x):
        #b, t, c, h, w = x.shape  # (2, 15, 1, 736, 1280) # (2, 15, 736, 1280)
        ...
        if(COL_CHANS==1): #Colour stacking
            #height, width = x.shape[-2::]
            #b, t, h, w = x.shape  # (2, 15, 224, 224)
            
            #num_stacks = N // self.stack_size
            x = x.view(B * num_stacks, self.stack_size, H, W)  # (10, 3, 224, 224)
        else:
            assert COL_CHANS==3
            x = x.view(B * N, COL_CHANS, H, W)

        #assert N % self.stack_size == 0
        #num_stacks = N // self.stack_size
        #x = x.view(B * self.num_stacks, self.stack_size, H, W)  # (10, 3, 736, 1280)
        ...
        #x = x.view(B, num_stacks, self.num_3d_features, h_down, w_down)  # (2, 5, 192, 23, 40)
        return x
